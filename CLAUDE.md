...
Protocol profiles live under `profiles/` and define wire-format constants for different addon interop layers. `McbewsV1Profile` is the sole built-in profile (module-level singleton `MCBEWS_V1`). MCBEWS/1 manifest resources (`manifest.json` and `vectors.json`) are authoritative for channel IDs, capability/session/framing/DDUI version axes, stable errors, and bounds; `protocol.ts` is the checked reference-Addon projection. The profile controls semantic bridge IDs, chat prefixes, trusted ToolPlayer name, chunk delays, typed session/approval DTOs, and the text-response frame format (`mcbews:text_resp`). `AddonBridgeSettings` carries the concrete profile reference so the addon service stays parameterised.
...
The bridge carries structured capability requests/responses over `scriptevent` via a simulated bridge player (`MCBEWS_BRIDGE`). Messages are piped through chat with a `namespace|prefix|request_id|index/total|content` format (`MCBEWS|BRIDGE`, `MCBEWS|UI_CHAT`) and UI Chat payloads retain `cid`. `AddonBridgeSession` handles chunk reassembly with TTL-based buffer expiry, byte limits, and per-connection future management. Text response usage is completion-frame-only; v1 session responses are atomic and return correlated `SESSION_RESPONSE_TOO_LARGE` errors when the configured command budget is exceeded. There is no global singleton — `AddonBridgeService` instances are constructed with explicit `AddonBridgeSettings`.
